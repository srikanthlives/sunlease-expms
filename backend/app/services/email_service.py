...
1. HTTP mail relay (scripts/cpanel-mail-relay.php) - used whenever
   EXPMS_MAIL_RELAY_URL is set. A small PHP script hosted on the SAME
   ...
   connection. This is the path actually in use on Railway, where direct
   outbound SMTP times out on both port 465 and 587.
2. Direct SMTP (stdlib smtplib, no third-party mail SDK) - used whenever
   EXPMS_MAIL_RELAY_URL is NOT set. Works fine for local/bare-metal dev
...
# included) - a hung/blocked outbound connection should fail fast with a
# real 502 from OUR app (with a useful message and a log line), rather than
# the platform's own proxy timing the whole request out first and returning
# a bare, undiagnosable 502 with no body.
_TIMEOUT_SECONDS = 12
...
def _send_via_relay(*, to_email: str, subject: str, body: str, attachment_bytes: bytes, attachment_filename: str, attachment_mime: str):
    ...
    if not settings.MAIL_RELAY_SECRET:
        _log("NOT SENT - EXPMS_MAIL_RELAY_URL is set but EXPMS_MAIL_RELAY_SECRET is empty")
        raise HTTPException(status.HTTP_503_SERVICE_UNAVAILABLE, "Email relay isn't fully configured - EXPMS_MAIL_RELAY_SECRET is missing.")
...
def _send_via_smtp(*, to_email: str, subject: str, body: str, attachment_bytes: bytes, attachment_filename: str, attachment_mime: str):
    _log(
    ...
    if not settings.SMTP_HOST or not settings.SMTP_FROM_EMAIL:
        ...
        raise HTTPException(
            ...
            "Email sending isn't configured on this server - set EXPMS_SMTP_HOST and EXPMS_SMTP_FROM_EMAIL, "
            "or EXPMS_MAIL_RELAY_URL/EXPMS_MAIL_RELAY_SECRET (see .env.example).",
        )
