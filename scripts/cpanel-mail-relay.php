 ...
 * instead (port 443 is never blocked). It then submits the message over
 * SMTP to mail.sunlease.in itself - which, being local traffic on this
 * server's own network, never crosses the boundary Railway's block applies
 * to.
 *
 * Originally this used PHP's mail() function instead of real SMTP, but
 * mail() turned out to be disabled outright on this cPanel account
 * ("Call to undefined function mail()") - some hosts lock it down and push
 * everyone onto authenticated SMTP submission instead. Hence the small
 * hand-rolled SMTP client below (implicit TLS on port 465 + AUTH LOGIN over
 * a raw socket) - no PHPMailer/Composer dependency needed for something
 * this small, and shared hosting often doesn't have Composer set up anyway.
 *
...
define('RELAY_SECRET', 'df35dc68276200db2eea35c6329fc1c1f5b56a6f9cde5a1ce7dabbd8ecb56a0467804759a450f67ae561e8a4a9439016fcee19316db6d255cfde4c38545215');
...
define('SMTP_USERNAME', 'expms@sunlease.in'); // usually same as FROM_EMAIL
define('SMTP_PASSWORD', 'sunlease@123');
...
// cPanel - which is exactly what showed up in production and gave zero clue
// what actually went wrong. This shutdown handler intercepts any fatal that
// happens after this point and turns it into a normal JSON error response,
// plus an error_log() line (visible in cPanel > Metrics > Errors) with the
// real cause.
register_shutdown_function(function () {
    ...
    error_log('[cpanel-mail-relay] FATAL: ' . $message);
    if (!headers_sent()) {
...
error_log(sprintf(
    '[cpanel-mail-relay] request received: %d bytes, memory_limit=%s, post_max_size=%s',
...
try {
    ...
    error_log('[cpanel-mail-relay] SMTP send failed: ' . $exc->getMessage());
