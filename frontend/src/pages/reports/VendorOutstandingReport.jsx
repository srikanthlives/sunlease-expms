...
export default function VendorOutstandingReport() {
  ...
  useEffect(() => {
    ...
    client.get("/reports/vendor-outstanding", { params }).then((res) => setRows(res.data));
  }, [range, projectId]);

  const totalInvoiced = rows ? rows.reduce((s, r) => s + r.invoiced, 0) : 0;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-display font-semibold">Vendor Outstanding</h1>
          <p className="text-sm text-ink/50 mt-0.5">What's owed to each vendor.</p>
        </div>
      ...
      {rows && (
        <>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            <StatCard label="Total Invoiced" value={formatMoney(totalInvoiced)} />
            <StatCard label="Total Paid" value={formatMoney(totalPaid)} tone="ok" />
            <StatCard label="Total Outstanding" value={formatMoney(totalOutstanding)} tone="warn" />
          </div>

          {rows.length > 0 && (
            <Card>
              <h2 className="font-display font-semibold text-lg mb-4">Paid vs Outstanding by Vendor</h2>
              <HorizontalBreakdownList
                data={rows} nameKey="vendor_name" sortBy="invoiced"
                segments={[
                  { key: "paid", label: "Paid", colorClass: "bg-ok" },
